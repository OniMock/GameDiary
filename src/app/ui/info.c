 ...
 static const char* s_version_result_lines[3];
static PopupData s_version_result_data;

static char s_formatted_msg_line[128];
static char s_formatted_version_line[64];
...
static void show_version_popup_advanced(MessageId msg_id, NetworkVersionInfo* info) {
    if (info && info->version[0] != '\0') {
        ...
        snprintf(s_formatted_version_line, sizeof(s_formatted_version_line), "V: %s", info->version);

        // Use standard label for date regardless of language to avoid adding another MSG mapping right now,
        // or just show the formatted date directly.
        snprintf(s_formatted_date_line, sizeof(s_formatted_date_line), "[ %s ]", date_str);
