...
static int power_callback(int unknown, int power_info, void *arg) {
    ...
    if (power_info & PSP_POWER_CB_POWER_SWITCH || power_info & PSP_POWER_CB_SUSPENDING) {
...
static int tracker_thread_main(SceSize args, void *argp) {
    ...
    cb_thid = sceKernelCreateThread("GameDiaryPwrCB", cb_thread, 0x30, 0x800, 0, 0);
    if (cb_thid >= 0) {
        sceKernelStartThread(cb_thid, 0, NULL);
    }
    
    // Initialize session
    storage_update_session(g_game_id, g_game_name, g_category, 0);
...
void tracker_thread_start(void) {
    running = 1;
    tracker_thid = sceKernelCreateThread("GameDiaryTrk", tracker_thread_main, 0x30, 0x1000, 0, 0);
    if (tracker_thid >= 0) {
