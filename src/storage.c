...
void storage_update_session(const char *game_id, const char *game_name, u8 category, u32 session_time) {
    const char *path = get_db_path();
    ...
    // Read existing database to memory (it is small enough, max 500 games * ~90 bytes = 45KB)
    // We allocate dynamically in kernel but safely, or we use a static buffer?
    // Let's use a static buffer to ensure absolutely zero memory fragmentation.
    // 500 entries = 46 KB static buffer.
    static GameDiaryEntry g_entries[500];
    
    SceUID fd = sceIoOpen(path, PSP_O_RDONLY, 0777);
    if (fd >= 0) {
        if (sceIoRead(fd, &header, sizeof(GameDiaryHeader)) == sizeof(GameDiaryHeader)) {
            if (header.magic == GAMEDIARY_MAGIC && header.version == DB_VERSION) {
                if (header.num_entries > 500) header.num_entries = 500;
    ...
    if (found_idx != -1) {
        ...
        if (session_time > 0) { // If it's a new session, but wait, updating every 60s shouldn't increment count!
            // Handled outside, we assume this function is called periodically with the *delta* session time.
            // Oh wait, if we only call this to add `session_time` which is the delta (e.g. 60 seconds).
            // No, the tracker provides `total session time elapsed`.
            // Let's reconsider: tracker should just pass `delta` to add.
            // Yes, tracking delta is easier.
        }
        ...
        if (header.num_entries < 500) {
            ...
            for(int i=0; i<3; i++) g_entries[found_idx].reserved[i] = 0;
        }
    }
