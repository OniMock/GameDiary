...
def extract_enum_keys(header_path):
    ...
    enum_match = re.search(r'typedef\s+enum\s*\{([^}]+)\}\s*MessageId\s*;', content, re.DOTALL)
...
def main():
    ...
    if missing_in_en:
        print(f"\n❌ FAILED: {len(missing_in_en)} keys in i18n.h but missing in i18n_en.c:")
    ...
    if missing_in_header:
        print(f"\n❌ FAILED: {len(missing_in_header)} keys in i18n_en.c but missing in i18n.h:")
    ...
    lang_files = [f for f in i18n_dir.glob("i18n_*.c")
    ...
    if not lang_files:
        ...
        print(f"Found {len(lang_files)} language data files: {', '.join(f.name for f in lang_files)}")
    ...
    for lang_file in lang_files:
        ...
        if missing_in_lang:
            print(f"\n❌ FAILED: {lang_file.name} is missing {len(missing_in_lang)} keys from i18n_en.c:")
        ...
        if extra_in_lang:
            print(f"⚠️  WARNING: {lang_file.name} has {len(extra_in_lang)} keys NOT in i18n_en.c:")
    ...
    if impl_file.exists():
        print(f"\nℹ️  INFO: Found implementation file: {impl_file}")
    ...
    if failed:
        ...
        print("\n✅ SUCCESS: All language files are consistent with i18n.h")
        sys.exit(0)
