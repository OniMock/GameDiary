## [1.0.0] - 2026-06-12

### 🐛 Bug Fixes

- *(plugin)* Improve  flicker-free overlay rendering
- *(plugin)* Red color in background text

### 🚀 Features

- *(ui)* Redesign sessions list layout and add column headers
- *(app)* Add Sessions screen with per-session deletion
- *(plugin)* Display tracker ON/OFF status on game startup when hotkey is enabled

<!-- generated by git-cliff -->
## [0.2.2] - 2026-05-30

### 🐛 Bug Fixes

- *(plugin)* Tracker overlay colors and remove unsafe display hooks
- *(ui)* Prevent carousel‑graph animation glitch on screen init

### 📚 Documentation

- Format README.md with improved list styling and consistent spacing
- Document plugin.dat and tracker.dat file specifications in README

### 🚀 Features

- *(game-details)* Add game deletion via triangle button
- *(ui)* Marquee scroll for truncated game names on selection
- *(ui)* Marquee scroll for truncated game names on selection
- *(ui)* Enhance plugin settings item rendering with selection highlight
- Embed CJK and Latin/Cyrillic font assets into the rendering engine

<!-- generated by git-cliff -->
## [0.2.1] - 2026-05-23

### 📚 Documentation

- Add release template with installation instructions and credits
- Add release template file for automated distribution metadata
- Mark UMD game plugin fix as completed in roadmap

### 🔧 Misc

- Bump project version from 0.2.0 to 0.2.1

### 🚀 Features

- *(ui)* Improve leaderboard layout and visibility

<!-- generated by git-cliff -->
## [0.2.0] - 2026-05-23

### 🐛 Bug Fixes

- *(plugin)* Delay physical UMD disc0 reads; keep ISO/POPS tracking stable
- *(ui)* Resolve language selection bugs and improve feedback

### 📚 Documentation

- Add compatibility list section and badge to README

### 🔧 Misc

- Bump version to 0.2.0 and update project name

### 🚀 Features

- *(utils)* Add 1 ms yield in file‑copy loop to prevent I/O starvation
- Add GitHub Actions workflow for PSP build, artifact deployment, and automatic releases
- Implement developed in splash screen UI and integrate static image resource management
- Change vps

<!-- generated by git-cliff -->
## [0.1.3] - 2026-05-12

### ♻️ Refactor

- Replace custom JSON parser with cJSON for version checking

### 🐛 Bug Fixes

- Github action

### 📚 Documentation

- Add documentation for official web tools and update export feature roadmap status

### 🔧 Misc

- Bump version to 0.1.3 and update project codename to Tassadar

### 🚀 Features

- *(ui)* Inform user of expected backup path when file is missing
- *(settings)* Add formatting menu with hours-only time display toggle
- Implement image resource management and add backup UI functionality with new icon assets

<!-- generated by git-cliff -->
## [0.1.2] - 2026-05-09

### ♻️ Refactor

- *(ui)* Separate informational screens from system settings

### ⚡ Performance

- *(font-pipeline)* Implement tight atlas packing and fix rendering UV coordinates

### 🎨 Styling

- *(ui)* Improve loading animation and fix popup layout responsiveness

### 🐛 Bug Fixes

- *(ui)* Improve text wrapping and layout safety in popups
- *(ui)* Resolve icon resize pop in main menu carousel
- *(ui)* Ensure total playtime displays days in all stats modes

### 📚 Documentation

- Update languages
- Update UI documentation in README
- Update localization and roadmap in README
- Add GitHub repository badges to README
- Enhance README with updated branding, badges, and project roadmap items

### 🔧 Misc

- Bump version
- Bump version

### 🚀 Features

- *(version)* Implement unified versioning and dynamic codename injection
- *(core)* Refine thread priorities and dynamic loading labels
- Mark dark/light theme UI expansion as complete in README
- Fix shadow in menu icon
- (ui) implement dynamic theme switching in settings
- Balance worker thread priority (0x20) and add CPU yielding to main loop.
- Add more debug to test
- Add GitHub Actions workflow for building debug
- Fix hardcoded in english
- *(font)* Update asset pipeline with multi-language SDF glyphs
- Clean up build dependencies, remove libs
- Implement debug logging system and remove unused network libraries from Makefile
- Remove unused objs
- Implement debug mode
- Add CI/CD workflow for building and releasing PSP project artifacts
- Add automated GitHub Actions build and release workflow with dynamic versioning
- Implement animated loading overlay component and screen manager integration
- Implement internationalization system for French support and font charset pipeline
- Add flags.svg asset for game localization support
- Add more Latin and Cyrillic font assets for rendering
- Implement internationalization system with language registry and image resource support (Italian)
- Add Italian language support to internationalization module

<!-- generated by git-cliff -->
## [0.1.1] - 2026-05-02

### ⚡ Performance

- *(render)* Implement dynamic 8-bit font loading for PSP 1000 memory safety

### 🐛 Bug Fixes

- *(render)* Resolve hardware-specific tearing and frame flashing during transitions
- *(boot)* Ignore inputs during splash to ensure proper initialization
- *(ui)* Improve i18n month labels and fix graph title overlap
- *(texture)* Add safety checks, correct GU setup, and optimize cache flush

### 📚 Documentation

- Update plugin installation instructions and expand feature roadmap in README

### 🚀 Features

- *(ui)* Add color parameters to nav indicators and optimize graph performance
- *(ui)* Add graphs details for each game
- Update icons
- Refactor icon_not_found asset and image resource
- Implement stats calculation logic and UI components for playtime visualization (Months)
- Refactor strings
- Mock database generation script

<!-- generated by git-cliff -->
## [0.1.0] - 2026-04-27

### ♻️ Refactor

- *(ui)* Centralize auto-fit text logic and fix activity layout overflows
- *(storage)* Replace sctrlKernelMsIsEf with robust device detection and centralize base path

### 🐛 Bug Fixes

- *(plugin)* Address PRO-C filesystem driver bugs with safety I/O fallbacks
- *(plugin)* Improve storage reliability for CFW 6.61 PRO-C
- *(ui)* Hide carousel ghost copies when filter has only one game
- Language key german

### 📚 Documentation

- Update file structure documentation to reflect sessions.dat and config.dat renaming

### 🔧 Misc

- Bump project version to 0.1.0 in Makefile
- Merge pull request [#2](https://github.com/OniMock/GameDiary/issues/2) from Narmo/ark4-installation-instructions
- *(giff)* Fix git-cliff configuration issue
- *(i18n-de)* Review German translations by Semmelstulle ([#1](https://github.com/OniMock/GameDiary/issues/1))

### 🚀 Features

- *(ui)* Replace game list "not found" placeholders with dynamic gray rectangles.
- Refactor project icon0
- *(app)* Implement async splash screen and optimized bulk data loading
- Add duplicate icon to simulate shadow
- *(plugin)* Implement self-exclusion to prevent tracking GameDiary app
- *(audio)* Implement procedurally generated SFX feedback system and UI hooks
- Add batch script to automate EBOOT.PBP and PRX deployment to PSP device
- Updated README.md
- *(i18n)* Sort language selection list alphabetically (A-Z)
- Implement resource embedding and internationalization support for German language assets

<!-- generated by git-cliff -->
## [0.0.3] - 2026-04-24

### 🐛 Bug Fixes

- *(ci)* Fix artifact upload
- *(plugin)* Fix storage save icon
- *(ci)* Update git-cliff arguments to prevent changelog duplication

<!-- generated by git-cliff -->

<!-- generated by git-cliff -->
## [0.0.1] - 2026-04-17

### ♻️ Refactor

- Automate build system and implement PNG texture loading support

### 🔧 Misc

- Add copyright on header
- Refactor github folder
- Add qrcode
- Add Readme
- Add MIT license
- Add git-cliff configuration for automated changelog generation

### 🚀 Features

- Add PSP export definition file and update gitignore to track it
- Add GitHub Actions workflow for automated PSP build and release process
- Add i18n key validation script
- Add missing strings
- Refactor i8n.h to another folder
- Refactor footer and top strings
- Refactor format time i8n
- Add bat to install eboot auto
- Implement game details screen for displaying statistics and metadata
- Refactor config.dat to root folder
- Refactor folder
- *(ui)* Add common geometric symbols as macros with hex escapes
- *(font-pipeline)* Always include special characters (®, °, ©, ™, etc.)
- Refactor strings
- Refactor format_time method, refactor build system to use isolated object directories.
- Implement core data loading, dashboard UI, internationalization, and utility functions for PSP file management.
- Implement UI screens, internationalization support, and font rendering system
- Implement internationalization system with language selection UI and multi-language support
- Implement internationalization system and add Spanish language support
- Fix string pt
- Add NotoSansCJK font and msdf-atlas-gen tool to project directory
- Implement game list, details, and settings screens with localization support
- Implement core UI rendering, localization system, and screen management architecture
- Implement core application architecture including data loading, UI screen management, and i18n support
- Refactor all code
- Add project-specific coding rules and architecture guidelines for PSP development
- Implement game metadata extraction and atomic storage system for PSP/PS1 icons
- Implement/fix persistent storage system for game registry and session logging
- Implement atomic storage system for game registry and session logging with integrity recovery
- Refactor persistent game session storage with atomic file updates
- Refactor game tracking core with SFO parsing, metadata repository, and session management logic
- Implement game detection logic for PSP, homebrew, and PS1 titles via PARAM.SFO and UMD parsing
- Implement game detection module and link kubridge library to detect ps1 games (id and name) on pops
- Add required libraries (ark-4)  to build configuration
- Implement game tracking logic with category detection (sceKernelInitApitype), PARAM.SFO parsing, and session storage
- Implement storage module for database initialization and session persistence
- Implement core game tracking, storage, and metadata detection modules for PSP
- Implement game time tracking and persistent storage system for PSP
- Implement game detection module to identify PSP titles and categories via SFO and UMD metadata
- Add comprehensive PSP SDK headers and initial project structure

<!-- generated by git-cliff -->
